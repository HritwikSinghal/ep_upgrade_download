...
class File_Crawler:
    ...
    def __get_file_list(self, folder_name: str):
        """
    ...
    def get_files(self):
        ...
        if not self.recursive_flag:
            file_list.extend(self.__get_file_list(self._file_dir))
        else:
            ...
            for (curr_dir, sub_dirs, files) in os.walk(self._file_dir, topdown=True):
                file_list.extend(self.__get_file_list(curr_dir))
